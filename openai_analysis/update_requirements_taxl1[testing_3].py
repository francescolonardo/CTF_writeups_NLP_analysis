...
new_requirements = """Given the input JSON provided, please label each substep. For each substep, assign the most appropriate category name to the 'Tier1TaxonomyMain' field from the list of 12 available categories. For the 'Tier1TaxonomyAlternative' field, select the second most appropriate category from the list of 12 available categories. If you're confident about the first category choice, use "N/A" for the 'Tier1TaxonomyAlternative' field's value.
To better understand each category, refer to the descriptions provided below:
```
We have 12 main categories, each with its own set of subcategories.
...
Evaluate each substep independently, without considering its preceding or following substep. Ensure you label using only the main category names, avoiding subcategories. When responding, please provide only the output JSON content without any additional information.
