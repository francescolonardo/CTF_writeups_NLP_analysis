...
def find_list_substeps_files_without_tag():
    list_substeps_filepaths = []
    for dirpath, dirnames, filenames in os.walk("./list_substeps"):
        for filename in filenames:
            if filename.startswith("list_substeps_") and filename.endswith(".json"):
                list_substeps_filepath = os.path.join(dirpath, filename)
                tagged_filepath = list_substeps_filepath.replace(
                    ".json", "_tagged.json"
                )
                if not os.path.exists(tagged_filepath):
                    substeps_filepaths.append(list_substeps_filepath)
...
if __name__ == "__main__":
    ...
    with open("./guidelines.json", "r", encoding="utf-8") as f:
        ...
        taxonomy_requirements = guidelines["requirements"]["requirements_tagged"]

    try:
        substeps_filepaths = find_list_substeps_files_without_tag()
