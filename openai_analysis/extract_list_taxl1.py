...
def collect_substep_strings(directory):
    substep_strings_reconnaissance = []
                    ...
                    with open(file_path, "r", encoding="utf-8") as json_file:
                        ...
                        for step in steps:
                            ...
                            for substep in substeps:
                                ...
                                taxonomy = substep.get("Taxonomy")
                                if substep_string and taxonomy:
                                    if taxonomy == "Reconnaissance":
                                        substep_strings_reconnaissance.append(
                                            substep_string
                                        )
                                    elif taxonomy == "Analysis":
                                        substep_strings_analysis.append(substep_string)
                                    elif taxonomy == "Exploitation":
...
# Collect substeps
(
    substeps_reconnaissance,
    substeps_analysis,
    substeps_exploitation,
) = collect_substep_strings(start_directory)

# Write substeps to output files
output_file_reconnaissance = "list_substeps_reconnaissance.json"
output_file_analysis = "list_substeps_analysis.json"
output_file_exploitation = "list_substeps_exploitation.json"

with open(output_file_reconnaissance, "w", encoding="utf-8") as output_json:
    json.dump(substeps_reconnaissance, output_json, ensure_ascii=False, indent=4)
...
print(f"Substeps collected and saved to `{output_file_reconnaissance}`")
